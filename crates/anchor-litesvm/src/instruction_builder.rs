//! DEPRECATED: Use anchor-client's Program API instead for IDL-based instruction building.
//!
//! This module is maintained for backward compatibility but new code should use
//! the production-ready syntax through ctx.program() which provides:
//! - IDL-based type safety
//! - Reusable client code between tests and production
//! - Automatic account and argument validation
//!
//! # Migration Example
//!
//! Instead of:
...
//!     .args(tuple_args((seed, receive, amount)))
...
///
/// **DEPRECATED**: This builder is maintained for backward compatibility.
/// New code should use anchor-client's Program API through `ctx.program()`
/// for IDL-based, type-safe instruction building that matches production syntax.
///
/// # Example (Deprecated)
///
/// ```ignore
...
/// ```
#[deprecated(since="0.2.0", note="Use anchor-client's Program API through ctx.program() for production-ready syntax")]
